...
const AdminOverviewPage = () => {
  ...
  // Helper function to extract clients array from various API response structures
  const extractClientsArray = (responseData) => {
    // console.log('Raw response data for extraction:', responseData); // Keep for debugging if needed
    ...
    if (responseData && typeof responseData === 'object') {
      ...
      for (const key of possibleArrayKeys) {
        if (Array.isArray(responseData[key])) {
          // console.log(`Found clients array in key: ${key}`); // Keep for debugging if needed
      ...
      if (allArrayKeys.length > 0) {
        // console.log(`Found arrays in keys: ${allArrayKeys.join(', ')}`); // Keep for debugging if needed
        return responseData[allArrayKeys[0]];
      }
    }

    // console.warn('No clients array found in response'); // Keep for debugging if needed
    return [];
  };

  // Calculates and sets the dashboard statistics
  ...
  useEffect(() => {
    ...
    const loadDashboardData = async () => {
      try {
        ...
        // console.log('Extracted clients array:', clientsArray); // Keep for debugging if needed
        // console.log('Number of clients extracted:', clientsArray.length); // Keep for debugging if needed

        if (mounted) { // Only update state if the component is still mounted
        ...
        if (mounted) {
          setError('Failed to load dashboard data. Please check if the backend server is running and getClients is correctly implemented.');
        }
      } finally {
        if (mounted) {
          setLoading(false);
    ...
    return (
      <div className="flex-1 p-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          ...
          <button
            onClick={() => loadDashboardData()} // Call the function to retry
          ...
          <>
            ...
            <div className="mt-4 text-center">
              <button
                ...
                onClick={() => nav('/agency-admin/dashboard/manage-clients')}
              >
                View All Clients
              </button>
            </div>
