...
// import { getClients, getClientAgencies, updateClientStatus } from "../../services/clientService";
...
//         <h1 className="text-xl font-semibold text-slate-800">Manage Clients</h1>
//         <input
...
export default function ManageClients() {
  ...
  const [agencies, setAgencies] = useState([]);
  const [saving, setSaving] = useState(false);
  const [updating, setUpdating] = useState(null); // Track which client is being updated
  ...
  const toggleActiveStatus = async (client) => {
    ...
    const updatedPayload = {
      ...
      IsVerified: client.IsVerified // Keep the same verification status
    };
